export function processJsonValue(options, ctx, value) {
	...
	if (Array.isArray(value)) {
		...
		if (valueIsNode) {
			...
			const locEntries = options.showLocations ? entries.filter(([key]) => key === 'position') : [];
			entries = entries.filter(([key]) => key !== 'type' && key !== 'position');
...
function isNode(value) {
	return value != null && 'position' in value && 'type' in value;
}
